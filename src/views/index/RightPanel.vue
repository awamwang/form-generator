<template>
  <div class="right-board">
    <el-tabs v-model="currentTab" class="center-tabs">
      ...
      <el-tab-pane label="数据逻辑" name="data" />
    </el-tabs>
    <div class="field-box">
      ...
      <el-scrollbar class="right-scrollbar">
        ...
        <el-form v-show="currentTab === 'field' && showField" size="small" label-width="90px">
          ...
          <el-form-item v-if="activeData.activeValueSubForm !== undefined" label="开启值子表">
            <el-select v-model="activeData.activeValueSubForm" :style="{ width: '100%' }">
              <el-option
                :label="sub.__config__.componentName"
                :value="sub.__config__.formId"
                :key="sub.__config__.formId"
                v-for="sub in subFormList"
              />
            </el-select>
          </el-form-item>
        ...
        <el-form v-show="currentTab === 'data'" size="small" label-width="90px">
          <template v-if="['sub-form'].indexOf(activeData.componentType) > -1">
            <el-divider>控制项</el-divider>

            <template v-if="controlList.length">
              <!-- TODO 表达或关系 -->
              <div v-for="(item, index) in activeData.__config__.controlItems" :key="index" class="reg-item">
                <span class="close-btn" @click="activeData.__config__.controlItems.splice(index, 1)">
                  <i class="el-icon-close" />
                </span>
                <el-form-item label="控制项">
                  <el-select
                    v-model="item.controlItem"
                    value-key="__config__.formId"
                    :style="{ width: '100%' }"
                    clearable
                    @change="contromItemChange(item)"
                  >
                    <el-option
                      :label="controlItem.__config__.label + controlItem.__config__.formId"
                      :value="controlItem"
                      :key="controlItem.__config__.formId"
                      v-for="controlItem in controlList"
                    />
                  </el-select>
                </el-form-item>
                <el-form-item label="控制逻辑" style="margin-bottom: 0;" v-if="item.controlItem">
                  <el-switch
                    v-model="item.valueEqual"
                    :active-value="item.controlItem['active-value']"
                    :inactive-value="item.controlItem['inactive-value']"
                    v-if="item.controlItem.__config__.tag === 'el-switch'"
                  />
                </el-form-item>
              </div>
              <div style="margin-left: 20px;">
                <el-button icon="el-icon-circle-plus-outline" type="text" @click="addControlItem">
                  添加控制项
                </el-button>
              </div>
            </template>
            <div v-else>没有可用的控制项</div>
          </template>
          <!-- <el-form-item label="控制项">
            <el-input v-model="formConf.formRef" placeholder="请输入表单名（ref）" />
          </el-form-item> -->
...
export default {
  ...
  props: ['showField', 'activeData', 'formConf', 'drawingList', 'subFormList'],
  data() {
    return {
      currentTab: 'field',
      currentNode: null,
  ...
  computed: {
    ...
    controlList() {
      console.log('controlList', this.drawingList)
      return this.drawingList.filter((e) => e.__config__.canHasSubForm)
    },
  },
  watch: {
    formConf: {
  ...
  methods: {
    ...
    addControlItem() {
      this.activeData.__config__.controlItems.push({
        controlItem: null,
        valueEqual: undefined,
      })
      console.log(this.activeData.__config__.controlItems)
    },
    contromItemChange(item) {
      if (item.controlItem.__config__.tag === 'el-switch') {
        item.valueEqual = item.controlItem.__config__.defaultValue
      }
    },
  },
}
